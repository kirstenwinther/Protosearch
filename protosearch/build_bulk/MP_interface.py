...
class MPInterface:
    ...
    from Materials Project structure prototypes
    ...
    def create_proto_data_set(self,
        ...
        unique Materials Project structure prototypes.
    ...
    def get_atoms_for_prototype(self,
        ...
        if self.required_elements:
            ...
            structures = list(mp_db.select(elements,
                                           proto_name=proto_name))
        else:
            structures = list(mp_db.select(proto_name=proto_name))
        ...
        for s in structures:
            ...
            for atoms in atoms_sub_list:
                ...
                #prototype_be = get_classification(atoms)
                #spacegroup = SPG.get_spacegroup()
                #new_species = prototype_be['species']
                #new_wyckoffs = prototype_be['wyckoffs']

                PC = PrototypeClassification(atoms)
                atoms = PC.atoms
                spacegroup = PC.spacegroup
                new_species = PC.species
                new_wyckoffs = PC.wyckoffs

                structure_name = str(spacegroup)

                for spec, wy_spec in zip(new_species, new_wyckoffs):
                ...
                loss_function = get_loss(atoms)

                atoms_data += [{'spacegroup': spacegroup,
                                'wyckoffs': list(new_wyckoffs),
        ...
        indices = [i for i in range(len(atoms_data))
                   ...
                   and not np.any(structure_names[i] in structure_names[:i])]
